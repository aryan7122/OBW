...
import { fetchSalonBlogDetail, fetchSalonBlogs } from '../../../../features/salon/salonBlogSlice';
...
const SalonBlogDetail = () => {
    ...
    const { detailLoading, blogDetail, detailError, apiblogs } = useSelector(state => state.salonBlog);
    console.log("📦 apiblogs", apiblogs);


    useEffect(() => {
        if (slug) {
            dispatch(fetchSalonBlogDetail(slug));
                    dispatch(fetchSalonBlogs());
            
        }
    }, [dispatch, slug]);
    console.log('api blogDetail', blogDetail)
    // const article = blogDetail //useMemo(() => blogDetail.find(b => b.url === slug) || NotFoundBlog, [slug]);

    // const nextArticle = useMemo(() => {
    //     const currentIndex = blogDetail.findIndex(b => b.url === slug);
    //     return currentIndex > -1 && currentIndex < blogDetail.length - 1 ? blogDetail[currentIndex + 1] : null;
    // }, [slug]);
    ...
    const handleShare = (platform) => {
        ...
        switch (platform) {
            ...
            case 'twitter': shareUrl = `https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodeURIComponent(blogDetail.title)}`; break;
    ...
    if (!blogDetail || Object.keys(blogDetail).length === 0) {
        return <p className="text-center">Blog not found or loading failed.</p>;
    }
        ...
        <div className="blog-detail-page">
            ...
            <header className="blog-detail-header">
                ...
                <div className='tags_page'>
                    {blogDetail.category && <div className='category '>{blogDetail.category}</div>}
                </div>
                <h1>{blogDetail.title}</h1>
                <WebPImage src={blogDetail.image} alt={blogDetail.title} className="cover-image" />
                <div className="metadata-bar">
                    <div className="publish-date">Published on <h4> {blogDetail.published_at}</h4></div>
                    ...
                    <div className='next_content'>
                        <div className="next-article-info">
                            <div className='tags_page_Next'>
                                ...
                                <div className='read-time'>{blog.published_at}</div>
                            </div>
                            <h3>{blog.title}</h3>
                        </div>
                        <div className="next-article-nav">
                            <button className='read_ ' onClick={() => navigate(`/blog-detail/${blog.url}`)}>

                                Read full blog
                                <ArrowRight className="arrow-icon" size={20} strokeWidth={2} />
                            </button>
                            <button className='next_ ' onClick={() => navigate(`/blog-detail/${blog.url}`)}>
